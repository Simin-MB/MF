...
class FriendBasedCF(object):
    def __init__(self, eta=0.5):
        ...
        self.sparse_training_matrix = None

    def compute_friend_sim(self, social_relations, sparse_training_matrix):
        ctime = time.time()
        print("Precomputing similarity between friends...", )
        self.sparse_training_matrix = sparse_training_matrix
        for uid in social_relations:
            for fid in social_relations[uid]:
                if uid < fid:
                    ...
                    u_check_in_neighbors = set(sparse_training_matrix[uid, :].nonzero()[0])
                    f_check_in_neighbors = set(sparse_training_matrix[fid, :].nonzero()[0])
    ...
    def predict(self, i, j):
        if i in self.social_proximity:
            numerator = np.sum([(self.eta * jf + (1 - self.eta) * jc) * self.sparse_training_matrix[k, j]
